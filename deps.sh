...
build_libunistring() {
  ...
  ./configure CFLAGS="-Os" --build=x86_64-pc-linux-gnu --host=$PREFIX --prefix=$INSTALLDIR --disable-shared --enable-static --disable-doc --disable-silent-rules --disable-tests || exit 1
  make -j$(nproc) || exit 1
  make install || exit 1
